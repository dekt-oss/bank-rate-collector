(()=>{
  ...
  const amount=value=>Number.isFinite(value)
    ?`${Number(value).toLocaleString("ko-KR",{maximumFractionDigits:1})}억원`:"—";
  ...
  function inlineData(){
    ...
    return JSON.parse(String(node.textContent||"").replace(/<\\\//g,"</"));
  }
  ...
  function decodedRows(table){
    ...
    return (table.rows||[]).map(row=>({
      ...
      max_rate:finite(get(row,"max_rate")),
  ...
  async function loadTable(){
    ...
    tablePromise=fetch(url,{cache:"no-store"})
      .then(response=>{
  ...
  function activeSectors(){
    const mutual=[...document.querySelectorAll("[data-sector]:checked")]
      .map(el=>el.dataset.sector).filter(Boolean);
    const selected=document.querySelector("[data-market-mode].active")?.dataset.marketMode||"savings_bank";
    if(selected==="savings_bank")return["savings_bank"];
    if(selected==="mutual_finance")return mutual;
  ...
  function buildSurface(context,proposal,inputs){
    ...
    const args={
      ...
      market_rows:context.marketRows,
      anchor_product_id:context.anchorId,
      current_own_rate:context.anchor.max_rate,
      proposal_rate:proposal,
      economics_min_rate:Math.max(0,Number((context.anchor.max_rate-SUPPORT_RADIUS_PP).toFixed(4))),
      economics_max_rate:Number((context.anchor.max_rate+SUPPORT_RADIUS_PP).toFixed(4)),
      baseline_new_money:inputs.baseline_new_money,
      maturity_amount:inputs.maturity_amount,
      current_rollover_rate_pct:inputs.current_rollover_rate_pct,
      term_months:context.term
    };
    const frame=PublicStructuralV2Surface.buildSurfaceFrame(
      args,
      PublicStructuralV2MarketPosition,
      MARKET_CONFIG,
      PublicStructuralV2DecisionContract
    );
    ...
    const forecast=PublicStructuralV2DecisionContract.buildPublicForecast({
    ...
    return PublicStructuralV2Surface.attachForecast(frame,forecast);
  }

  function positionFor(context,rate){
    return PublicStructuralV2MarketPosition.marketPosition({
  ...
  const forecastAt=(surface,rate)=>(surface.forecast?.scenarios||[])
    .find(row=>Math.abs(Number(row.rate_pct)-rate)<.00005)||null;
  const positionAt=(surface,rate)=>(surface.market_positions||[])
    .find(row=>Math.abs(Number(row.proposal_rate)-rate)<.00005)||null;
  ...
  function nearbyHtml(context,review){
    const rows=context.products
      .map(row=>({...row,gapBp:(row.max_rate-review)*100,absGapBp:Math.abs((row.max_rate-review)*100)}))
  ...
  function peerHtml(review){
    ...
    const peers=rp.peers
      .map(peer=>({...peer,rate:finite(peer.rate_pct)}))
      .filter(peer=>peer.rate!==null)
      .map(peer=>({...peer,gap:(peer.rate-review)*100}))
  ...
  function renderFacts(context,rate,surface){
    ...
    $("rds-rank").textContent=rankText(position);
    $("rds-threshold").textContent=position
      ?`TOP10 ${pct(position.top10_cutoff)} · TOP25 ${pct(position.top25_cutoff)} · 중앙 ${pct(position.median_rate)}`
      :"시장 위치 근거 없음";
  ...
  async function render(){
    ...
    try{
      ...
      const surface=buildSurface(context,context.anchor.max_rate,inputs);
      const selection=StrategyTargetCandidate.findFirstCandidate(surface.forecast?.scenarios||[],target);
      if(selection.status!=="ready"){
        clearUnsupported(target);
        return;
      }
  ...
  function install(){
    ...
    if(!panel||$("strategy-rate-decision-simulator"))return;
    const host=document.createElement("section");
    host.id="strategy-rate-decision-simulator";
    host.className="strategy-rate-decision-simulator";
    ...
    host.querySelectorAll("[data-rds-mode]").forEach(button=>button.addEventListener("click",()=>{
      ...
      host.querySelectorAll("[data-rds-mode]").forEach(item=>item.classList.toggle("active",item===button));
      host.querySelector(".rds-rate-mode").hidden=mode!=="rate";
    ...
    document.querySelectorAll("[data-market-mode],[data-sector],#term-segment button").forEach(el=>
      el.addEventListener("click",()=>setTimeout(render,50))
    );
