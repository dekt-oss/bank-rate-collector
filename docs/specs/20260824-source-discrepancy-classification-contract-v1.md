...
대신 현재 규칙을 **경계값과 precedence까지 자동 테스트로 잠근 뒤**, 동일 계약으로 현재
production mismatch 6건을 다시 생성하여 A1/A2 forensic 대상을 확정한다.
...
## 5. Current queue replay contract

현재 production queue의 대표 두 계열을 fixture 수준에서 재생한다.

### 대신저축은행 계열

- 현재 queue: 정기적금 24m / 36m
...
- 현재 queue: 정기예금 12m branch simple/compound 및 비대면 simple/compound
...
A1/A2 착수 전 production R2 read-only audit으로 실제 6건을 다시 생성해야 한다.
...
10. 대신/DH 현재 queue class family representative replay
...
A0 CI 및 production read-only audit가 동일한 규칙/queue를 확인하면:
