...
마이그레이션 `c93f5b71e284`. 2026-08-06 rate-data `60e90e7` 사본에 적용해
대조했다 (16초).

| | 전 | 후 |
|---|---:|---:|
| 관측 | 185,923 | **145,058** |
| 살아 있는 행 | — | 130,131 (비교 단위 수와 같다) |
| 닫힌 이력 | — | 14,927 |
| `seen_count` 합계 | — | **185,923** (원래 행 수와 같다) |
| 발행본 | 275,714,048 B | **247,488,512 B** |
| 금리표 | 132,502행 | 132,502행 |
| `validate` | 12/12 | 12/12 |

`seen_count` 합계가 원래 행 수와 같다는 것이 불변식이다 — 행은 접혀도 몇 번
봤는지는 잃지 않는다.

화면은 `run_id`가 아니라 `last_run_id`로 건다. 관측이 처음 본 실행에 묶여
있으므로 `run_id`로 걸면 안 바뀐 금리가 통째로 사라진다.

발행 스냅샷이 `backup()` 대신 `VACUUM INTO`를 쓴다. DELETE만으로는 파일이
줄지 않아, 4만 행을 지우고도 287 MB가 445 MB로 늘었다.
