...
### 11.1 정정 — 구·군 이름 목록을 걷어낸다 (2026-08-05)

위 형태는 부산 16개 구·군을 `config/regions.yaml`에 손으로 적어두는 것을 전제했다. 실측으로 그 전제가 무너졌다.

- `list.do?r1={지역}&r2=`처럼 **`r2`를 빈 문자열로 주면 그 지역 전체가 한 응답에 온다** (부산 = 273점포/137금고, 1회). 시군구 목록이 애초에 필요 없다.
- `r1`은 행정구역 시도가 아니라 사이트의 지역본부 묶음이다. `r1=광주`가 전남 주소 124건을 함께 돌려준다. 이 값을 시도로 쓰면 기관이 엉뚱한 지역에 붙는다.

그래서 `config/regions.yaml`은 구·군 이름 대신 **수집 범위(`scopes`)** 를 담는다. 부산은 수집 단위가 아니라 그 범위 중 하나다.
...
**바뀌지 않는 것**: 행정구역 공식 코드는 여전히 `null`이고, `validation_service`가 이 칸이 채워지면 실패시킨다. 지역 표시는 언제나 점포 주소에서 뽑는다 (`parser.split_region`).
...
### 12.4 수집 없는 발행(publish-only) 실행의 게이트 범위 (2026-08-07 추가)

2026-08-07에 **수집을 건너뛰고 발행만 하는 실행 모드**가 생겼다. main에 머지하면
화면이 바로 바뀌게 하려는 것이다 (`.github/workflows/collect.yml`의 `push` 방아쇠와
`publish_only` 입력). 이 실행은 원천에 접속하지 않고, 직전 DB를 복원해 화면만 다시 만든다.

§12.1의 게이트는 **수집이 방금 일어났다는 전제로 쓰였다.** 항목 셋이 러너 디스크의
`data/raw/`를 세는데, 수집이 없으면 그 자리는 비어 있다. 원본이 사라진 것이 아니라
이번 실행이 원본을 만들지 않은 것이다. 실제로 run 25·26이 여기서 멈춰 발행 단계까지
건너뛰었고, 머지해도 화면이 안 바뀌었다.

그래서 publish-only 실행에서는 아래 셋을 **검사하지 않고 건너뛴 사실을 적는다.**

| 항목 | 왜 건너뛰나 |
|---|---|
| 원본 보존 | 이번 실행이 원본을 만들지 않았다. 디스크가 빈 것이 정상이다 |
| max_rate 대조용 finlife 원본 확보 | 대조할 원본 파일이 이번 실행에 없다 |
| max_rate NULL 규칙 — finlife (원본 대조) | 위와 같은 이유. **지금까지 0 == 0으로 조용히 통과하고 있었다** — 이쪽이 더 나쁘다 |

**통과시키지 않고 건너뛴다.** 같은 파일이 이미 쓰는 `[건너뜀]` 표기를 그대로 쓴다
(원천을 안 돌렸을 때와 같은 처리). 0 == 0으로 통과시키면 검사가 아니라 장식이 된다.

나머지 항목은 전부 그대로 돈다. 발행되는 DB·manifest·화면을 검사하는 항목들은
수집 여부와 무관하기 때문이다. 수집이 도는 실행에서는 셋 다 예전처럼 검사한다.

---

## 12.5 화면에서 바로 수집을 시작한다 (2026-08-08 추가)

**바뀌는 계약.** 지금까지 화면의 «지금 수집하기»는 GitHub Actions 실행 화면으로
보내는 **링크**였다. 누르면 GitHub으로 이동해 거기서 다시 입력칸을 채우고 버튼을
눌러야 했다. 앞으로는 화면에서 암호를 넣으면 그 자리에서 수집이 시작된다.

### 왜 링크였나, 왜 바꾸나

v3.1 §2가 로컬 FastAPI를 후순위로 내리면서 이 사이트는 서버가 없어졌다. 정적
파일만 있으므로 페이지 안에서 GitHub API를 부르려면 토큰이 필요하고, 토큰을
페이지에 실으면 그 순간 아무나 쓸 수 있다. 그래서 링크로 두었다.

바꾸는 것은 그 판단이 틀려서가 아니라 **토큰을 둘 자리가 생겨서**다. 화면을
내주는 Vercel이 서버리스 함수를 같은 도메인에서 함께 내준다. 토큰은 그 함수의
환경변수에 있고 브라우저로 내려가지 않는다.

### 경로

```
브라우저  ──POST /api/collect {password, 옵션}──▶  Vercel 함수
                                                    │ ① 암호 대조 (COLLECT_PASSWORD)
                                                    │ ② 이미 도는 실행이 있으면 거절
                                                    │ ③ workflow_dispatch 호출
                                                    ▼
                                                 GitHub Actions
                                                    │ ④ 암호 재대조 (DASHBOARD_PASSWORD)
                                                    ▼
                                                  수집
```

**암호는 한 곳에만 둔다.** GitHub Actions 시크릿 `DASHBOARD_PASSWORD` 하나가
유일한 정답이고, 함수는 받은 값을 그대로 실어 보낸다. 대조는 ④에서만 일어난다.

Vercel에도 같은 값을 넣어 ①에서 한 번 더 대조하면 방어는 한 겹 두꺼워진다.
그 길을 택하지 않은 이유는 **같은 비밀을 두 곳에 두면 언젠가 한쪽만 바뀌기**
때문이다. 그날 화면은 «암호가 틀렸다»고 하고 GitHub은 맞다고 하는데, 어느
쪽이 진짜인지 화면으로는 알 수 없다.

대가가 있다. 틀린 암호도 실행을 하나 만든다. 그 실행은 암호 단계에서 10초쯤
만에 죽지만, 막지 않으면 이 주소가 그대로 무제한 추측기가 된다. 그래서
**시도 자체를 센다** (아래).

화면이 받는 «시작했습니다»는 **«암호가 맞았다»는 뜻이 아니다.** 대조가 그
실행 안에서 일어나므로, 틀리면 몇 초 뒤 목록에서 빨간 X로 끝난다. 응답 문구가
그렇게 적어야 보는 사람이 확인하러 간다.

암호가 로그에 남지 않는 것은 그대로다. GitHub이 등록된 시크릿과 같은 문자열을
가린다 — run 31232386844 로그 실측에서 `GIVEN: ***`으로 나온다. 틀린 값은
가려지지 않지만, 그건 추측한 쪽이 스스로 적어 넣은 문자열이다.

### 남용을 막는 것

이 사이트는 인증이 없고 주소를 아는 사람은 누구나 연다. 함수가 인터넷에 열리므로
암호 추측과 수집 도배를 둘 다 막아야 한다. 저장소를 새로 두지 않고 GitHub에
물어서 판단한다 — 상태를 우리가 들고 있지 않아야 함수가 여러 벌 떠도 일관된다.

| 막는 것 | 방법 |
|---|---|
| 암호 추측 | 한 시간에 수동 실행 `MAX_ATTEMPTS_PER_WINDOW`(5)회를 넘으면 거절 |
| 빈 암호 | 함수가 끊는다. GitHub까지 보내면 실행 하나를 버리고 그것이 위 계산에 들어간다 |
| 수집 도배 | 이미 진행 중이거나 대기 중인 수집이 있으면 거절 |
| 잦은 재수집 | 마지막 **성공한** 수집이 `COLLECT_MIN_INTERVAL_MINUTES`(기본 30분) 안이면 거절 |

간격 제한이 «성공한» 수집만 세는 것이 중요하다. 암호가 틀려 10초 만에 죽은
실행까지 세면 오타 한 번에 30분을 기다리게 된다. main 푸시로 도는 실행도
수집이 아니라 발행(2분)이라 뺀다 — 그것 때문에 막으면 머지 직후에는 아무도
수집을 못 돌린다.

시도 계산은 반대로 **성공·실패를 가리지 않는다.** 실패만 세면 맞는 암호를
사이에 섞어 계산을 지울 수 있다.

**그래서 짧은 숫자 암호도 쓸 만하다.** 시간당 다섯 번이 상한이므로, 여섯 자리
숫자(100만 가지)를 다 훑으려면 평균 10만 시간이 걸린다. 암호의 길이가 아니라
시도 횟수가 벽이 된다.

전국 수집은 3시간 41분이 걸린다. 두 벌이 동시에 돌면 `concurrency` 그룹이
줄을 세우므로 데이터가 깨지지는 않지만, 원천에 두 배로 요청이 간다. 시작 전에
막는 편이 낫다.

### 토큰

`GITHUB_DISPATCH_TOKEN`은 **이 저장소 하나에만, Actions 쓰기 권한만** 주는 세밀한
접근 토큰(fine-grained PAT)이어야 한다. 워크플로를 돌리는 것 말고는 아무것도 못
하는 토큰이라야 새더라도 피해가 «누가 수집을 돌렸다»에서 멈춘다.

### 설정값

함수는 아래를 환경변수로 받는다. 하나라도 없으면 **기능을 끄고** 그 사실을 응답에
적는다 — 반쯤 켜진 채로 «되는 줄 알았는데 안 되는» 상태를 만들지 않는다.

| 이름 | 무엇 | 손으로 넣나 |
|---|---|---|
| `GITHUB_DISPATCH_TOKEN` | 위의 세밀한 접근 토큰 | **넣어야 한다** |
| `GITHUB_REPOSITORY` | `소유자/저장소` | 대개 안 넣어도 된다 |

저장소 이름은 Vercel이 이미 안다. `VERCEL_GIT_REPO_OWNER`·`VERCEL_GIT_REPO_SLUG`가
환경에 들어오므로 그것으로 맞춘다. 다만 그 노출은 프로젝트 설정(System
Environment Variables)에 달려 있어 꺼져 있을 수 있고, 그때는 `GITHUB_REPOSITORY`를
넣는다. 명시값이 있으면 그쪽이 이긴다.

**결국 손으로 넣는 것은 토큰 하나다.** 토큰은 줄일 수 없다 — GitHub이 이름
없는 요청으로 워크플로를 돌려 주지 않기 때문이다.

암호는 여기 없다. `DASHBOARD_PASSWORD`(GitHub 시크릿)를 그대로 쓴다.

`COLLECT_MIN_INTERVAL_MINUTES`는 선택이고 기본 30이다.

### 링크는 남긴다

함수가 없거나(설정 전) 꺼져 있으면 화면은 예전처럼 GitHub 링크를 보여준다.
전환 중에 아무 길도 없는 상태를 만들지 않기 위해서다.
