...
보존기간은 `retention-days: 400`을 목표로 설정한다. **계정·조직 정책상 상한이 더 낮을 수 있으므로, 설정 후 실제 적용값을 확인한다.** 400이 적용되지 않으면 허용 최대값으로 낮추고 그 값을 이 절에 기록한다.
...
| `rate-data` 브랜치 누적 용량 | P1-A 종료 시 실측 후 정리 정책 결정 |
| Artifact `retention-days` 실제 적용값 | §4에 실측 기입 필요 |
| private 저장소 Pages 공개 범위 | §5 옵션 경로 결정 전제 |
| finlife HTTPS 지원 여부·일일 호출 한도 | P0에서 이월 |
