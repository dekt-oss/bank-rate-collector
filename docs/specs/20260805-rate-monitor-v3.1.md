...
**암호를 두 번 본다.** ①은 함수가, ④는 워크플로가 본다. 함수의 토큰이 새더라도
④가 남고, ④만 있으면 브라우저에서 시작할 수 없다. 두 곳에 같은 값을 넣는다.

암호를 워크플로 입력으로 넘기는 것은 안전하다. GitHub이 등록된 시크릿과 같은
문자열을 로그에서 가린다 — run 31232386844 로그 실측에서 `GIVEN: ***`으로 나온다.
단 **틀린 암호는 가려지지 않는다.** 그래서 함수가 먼저 대조해, 틀린 값이 애초에
GitHub까지 가지 않게 한다.
...
| 막는 것 | 방법 |
|---|---|
| 암호 추측 | 길이가 같아야 비교하는 상수시간 대조. 틀리면 1초 지연 후 같은 문구로 응답 |
| 수집 도배 | 이미 진행 중이거나 대기 중인 수집이 있으면 거절 |
| 잦은 재수집 | 마지막 수집 시작이 `COLLECT_MIN_INTERVAL_MINUTES`(기본 30분) 안이면 거절 |
...
함수는 셋을 환경변수로 받는다. 하나라도 없으면 **기능을 끄고** 그 사실을 응답에
적는다 — 반쯤 켜진 채로 «되는 줄 알았는데 안 되는» 상태를 만들지 않는다.

| 이름 | 무엇 |
|---|---|
| `COLLECT_PASSWORD` | 화면에서 받을 암호. `DASHBOARD_PASSWORD`와 같은 값 |
