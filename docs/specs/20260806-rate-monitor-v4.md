...
`신동해빌딩 1,2,3층`이라 옛 SQL은 구·군을 `''`로, 새 규칙은 `1,2,3층`으로
읽는다. 둘 다 진짜 구가 아니고, 이제 `review_items`의
`region_unknown_sido`로 세어진다 (13건).

`region_unresolved` 927건은 주소가 원래 없는 원천이다 — 신협 848건은
조회조건만 알고, finlife 79건은 전국 공시다. 실패가 아니라 사실이다.
