# 명세서: 2금융권 수신금리 통합 모니터 (rate-monitor) v2

- 프로젝트명: `rate-monitor`
- 작성일: 2026-08-05
- 상태: 구현 기준 초안
- 관련 문서: `docs/plans/20260805-rate-monitor-plan-v3.md`
...
- 저장소는 JSON이 아니라 **SQLite**를 사용한다.
- 수집기는 `fetch → raw 저장 → parse → normalize → validate → persist` 단계로 분리한다.
- 자동·브라우저·파일 수입을 동일한 소스 어댑터 인터페이스로 처리한다.
- 기관, 지점, 상품, 상품옵션, 시점별 금리, 우대조건, 수기 편집을 별도 테이블로 관리한다.
- 화면값은 `최신 정상 관측값 + 활성 오버라이드`로 계산한다.
- 수집 실패 시 이전 정상값을 삭제하지 않는다.
- 농축협은 P0에서 중앙 전수 수집 가능성을 확인할 때까지 `partial` 소스로 구현한다.
...
1. 지역 목록 조회
2. 부산 구·군별 금고·점포 목록 조회
3. 금고·점포 공식 코드, 주소, 유형 저장
4. 금리 상세 페이지 조회
5. 상품·기간·금액구간 파싱
...
- 요구불예탁금 금액구간 분리
- 기준일 파싱
- 상품별 중도해지·만기후 이율은 본 비교금리와 별도 부가정보로 저장

기관 결정:

- 공식 금고 코드가 동일하면 같은 기관
- 점포 코드만 다르면 `outlet`
- 공식 코드가 없을 때만 이름·주소 매칭
...
P0 정찰 전에는 검증되지 않은 자동 수집원을 기본 비활성화한다.
...
이 다섯 항목이 없으면 실제 수집기 구현을 시작하지 않는다. 특히 실물 표본 없이 HTML·엑셀 파서를 추정 구현하지 않는다.
