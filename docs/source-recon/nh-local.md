...
- **이용약관.** 사용자가 직접 확인했고 자동수집을 제한하는 조항이 없었다.
  그래서 `NhLocalAdapter.policy_status = "allowed"`다. 새마을금고·신협이
  `review`인 것과 다른 이유는 이것 하나다 — 확인한 사람이 있다.
- **`geo_basis`.** `outlet_address`로 확정했다. 같은 조합의 지점들이 서로
  다른 주소를 갖는다(대저농협 3지점 실측). 조합 단위였다면 같아야 한다.
  `region_service.SOURCE_GEO_BASIS`에 들어가 있다.

### 아직 모르는 것

- **입출금식 화면 `SFDPW0162R`의 실물.** 거치식·적립식만 fixture가 있다.
  본 적 없는 표의 열 위치를 맞다고 가정하지 않으므로 수집에서 뺐다
  (`adapter.DEFAULT_PRODUCTS`). fixture를 받으면 넣는다.
...
- **전국을 실제로 돌려 본 적이 없다.** 4,871점포 × 2화면 = 9,743회, 1초
  간격이면 2시간 43분이다. 원천이 그 부하를 어떻게 받는지 모른다. 그래서
  기본 범위가 부산이다 (`config/regions.yaml`의 `nh_local_default_scope`).
