...
def test_strict_7d_keeps_deposit_but_drops_unknown_vs_f_s_savings(tmp_path: Path) -> None:
    ...
    assert coverage["fsb"]["installment_savings"]["known_payment_method"] == 0
    assert coverage["finlife_savings_bank"]["installment_savings"]["known_payment_method"] == 2

    transition = impact["ambiguity_transition"]["counts"]
    assert transition == {"strict7d_turns_into_payment_source_only": 1}


def test_same_7d_different_rates_remains_ambiguous(tmp_path: Path) -> None:
    db, report = _fixture(tmp_path)
    conn = sqlite3.connect(db)
    conn.execute(
        "INSERT INTO rate_observations VALUES "
        "('o6', 's-sav-s', 's', '3.5', '3.5', '2026-08-20', 'valid', NULL)"
    )
    conn.commit()
    conn.close()
    ...
    assert impact["strict_7d_simulation"]["total_ambiguous_secondary"] == 1
    example = impact["strict_7d_simulation"]["ambiguous_same_7d_examples"][0]
    assert example["key7"][-1] == "s"
    assert example["secondary"]["ambiguous"] is True
    assert len(example["secondary"]["rate_pairs"]) == 2
