...
def test_market_intelligence_briefing_is_injected_once() -> None:
    ...
    assert "▲ 인상" in rendered
    assert "▼ 인하" in rendered
    assert "─ 유지" in rendered
    assert "인상-인하 균형" in rendered
    assert "금리변경 breadth" not in rendered
    assert "변경상품 구성" in rendered
