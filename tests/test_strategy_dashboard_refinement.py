...
def test_prediction_engine_is_hidden_until_explicit_button_click() -> None:
    ...
    assert 'button.textContent=opening?"예측엔진 닫기":"예측엔진 보기"' in html
    assert '$("prediction-toggle").addEventListener("click",togglePredictionPanel)' in html
...
def test_prediction_engine_explains_model_and_evidence_scope() -> None:
    ...
    assert "https://www.federalreserve.gov/pubs/feds/2013/201380/index.html" in html
    assert "the-rise-in-deposit-flightiness-and-its-implications-for-financial-stability" in html
...
def test_national_map_gives_more_width_to_map_and_offsets_labels() -> None:
    ...
    assert ".primary:not(.busan-focus) .tablewrap table{min-width:540px}" in html
...
def test_national_map_refinement_does_not_change_busan_focus_contract() -> None:
    ...
    assert ".primary.busan-focus{grid-template-columns:minmax(720px,1.45fr)" in html
    assert 'document.querySelector(".primary")?.classList.add("busan-focus")' in html
    assert 'document.querySelector(".primary")?.classList.remove("busan-focus")' in html
