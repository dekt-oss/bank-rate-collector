    ...
    try:
        conn.executescript(
            ...
            INSERT INTO institutions VALUES
                ('i-covered', 'savings_bank', '테스트저축은행', '테스트저축은행');
            INSERT INTO institutions VALUES
                ('i-fallback', 'savings_bank', '보완저축은행', '보완저축은행');

            INSERT INTO products VALUES
                ('p-fsb', 'i-covered', 'term_deposit', '대표예금');
            INSERT INTO products VALUES
                ('p-fin', 'i-covered', 'term_deposit', '대표예금 FIN');
            INSERT INTO products VALUES
                ('p-only', 'i-fallback', 'term_deposit', '보완예금');
            ...
            INSERT INTO collection_runs VALUES
                ('r-fsb-old', 'fsb', datetime('now', '-2 day'), datetime('now', '-2 day'), 'success');
            INSERT INTO collection_runs VALUES
                ('r-fin-old', 'finlife_savings_bank', datetime('now', '-2 day', '+1 minute'), datetime('now', '-2 day', '+1 minute'), 'success');
            INSERT INTO collection_runs VALUES
                ('r-fsb-new', 'fsb', datetime('now', '-1 day'), datetime('now', '-1 day'), 'success');
            INSERT INTO collection_runs VALUES
                ('r-fin-new', 'finlife_savings_bank', datetime('now', '-1 day', '+1 minute'), datetime('now', '-1 day', '+1 minute'), 'success');

            INSERT INTO rate_observations VALUES
                ('o-fsb-old', 'v-fsb', 'r-fsb-old', datetime('now', '-2 day'), datetime('now', '-1 day'), 3.00, 'valid');
            INSERT INTO rate_observations VALUES
                ('o-fsb-new', 'v-fsb', 'r-fsb-new', datetime('now', '-1 day'), NULL, 3.20, 'valid');
            INSERT INTO rate_observations VALUES
                ('o-fin-old', 'v-fin', 'r-fin-old', datetime('now', '-2 day'), datetime('now', '-1 day'), 4.00, 'valid');
            INSERT INTO rate_observations VALUES
                ('o-fin-new', 'v-fin', 'r-fin-new', datetime('now', '-1 day'), NULL, 4.50, 'valid');
            INSERT INTO rate_observations VALUES
                ('o-only-old', 'v-only', 'r-fin-old', datetime('now', '-2 day'), datetime('now', '-1 day'), 3.70, 'valid');
            INSERT INTO rate_observations VALUES
                ('o-only-new', 'v-only', 'r-fin-new', datetime('now', '-1 day'), NULL, 3.90, 'valid');
            """
        )
        conn.commit()
    finally:
...
def test_strategy_history_uses_public_source_precedence(tmp_path: Path) -> None:
    ...
    assert summary["rate_trend"]["scope"]["source_precedence"] == (
        "presentation.db_only_sources"
    )
