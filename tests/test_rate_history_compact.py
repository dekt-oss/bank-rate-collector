...
def test_apply_collapses_only_consecutive_equal_values_and_preserves_current(
    ...
    assert rows[1][0:3] == ("o4", "r3", "r4")
    assert rows[1][3] == "2026-09-03"
    assert rows[1][4] == "2026-09-05"
    assert rows[1][6] == "2026-09-03"
    assert rows[1][5] == 4
...
def test_compaction_rejects_orphan_reference_before_planning(tmp_path: Path) -> None:
    db = tmp_path / "rates.sqlite3"
    _build_db(db)
    conn = sqlite3.connect(db)
    try:
        conn.executescript(
            ...
            CREATE TABLE preference_conditions (
              id TEXT PRIMARY KEY,
              observation_id TEXT NOT NULL REFERENCES rate_observations(id)
            );
            INSERT INTO preference_conditions(id, observation_id)
            VALUES ('orphan', 'missing-observation');
            """
        )
        conn.commit()
    finally:
    ...
    with pytest.raises(RuntimeError, match="source copy failed integrity checks"):
        compact_rate_history(db)


def test_current_observation_reference_survives_compaction(tmp_path: Path) -> None:
    db = tmp_path / "rates.sqlite3"
    ...
    try:
        ...
        conn.executescript(
            ...
            INSERT INTO preference_conditions(id, observation_id) VALUES ('p-current', 'o4');
            """
        )
        conn.commit()
    finally:
    ...
    report = compact_rate_history(db, apply=True, vacuum_into=candidate)
    assert report["reference_blockers"] == []
    conn = sqlite3.connect(candidate)
    try:
        assert conn.execute(
            "SELECT observation_id FROM preference_conditions WHERE id='p-current'"
        ).fetchone() == ("o4",)
        assert conn.execute(
            "SELECT id FROM rate_observations WHERE valid_to IS NULL"
        ).fetchone() == ("o4",)
