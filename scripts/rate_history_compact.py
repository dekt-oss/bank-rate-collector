...
def _current_fingerprint(conn: sqlite3.Connection) -> tuple[int, str]:
    ...
    rows = conn.execute(
        """
        SELECT id, variant_id, base_rate, max_rate, raw_preference_text,
...
def _create_plan(conn: sqlite3.Connection) -> None:
    conn.executescript(
        ...
        CREATE TEMP TABLE _compact_groups (
          ...
          first_valid_from TEXT NOT NULL,
          first_seen_at TEXT NOT NULL,
          first_run_id TEXT NOT NULL,
          first_raw_artifact_id TEXT NOT NULL,
          last_valid_to TEXT,
          last_as_of TEXT,
        ...
        SELECT
          ...
          grp,
          COALESCE(
            MAX(CASE WHEN valid_to IS NULL THEN id END),
            MAX(CASE WHEN rn_first = 1 THEN id END)
          ) AS keep_id,
          ...
          MAX(CASE WHEN rn_first = 1 THEN valid_from END) AS first_valid_from,
          MAX(CASE WHEN rn_first = 1 THEN first_seen_at END) AS first_seen_at,
          MAX(CASE WHEN rn_first = 1 THEN run_id END) AS first_run_id,
          MAX(CASE WHEN rn_first = 1 THEN raw_artifact_id END) AS first_raw_artifact_id,
...
def _apply_plan(conn: sqlite3.Connection) -> None:
    ...
    conn.execute(
        ...
        SET
          ...
          valid_from = (
            SELECT grp.first_valid_from
            FROM _compact_groups grp
            WHERE grp.keep_id = keep.id
          ),
          first_seen_at = (
            SELECT grp.first_seen_at
            FROM _compact_groups grp
            WHERE grp.keep_id = keep.id
          ),
          run_id = (
            SELECT grp.first_run_id
            FROM _compact_groups grp
            WHERE grp.keep_id = keep.id
          ),
          raw_artifact_id = (
            SELECT grp.first_raw_artifact_id
            FROM _compact_groups grp
            WHERE grp.keep_id = keep.id
          ),
          valid_to = (
...
def compact_rate_history(
    ...
    *,
    ...
    try:
        ...
        integrity_before, fk_before = _integrity(conn)
        if integrity_before != "ok" or fk_before:
            raise RuntimeError(
                f"source copy failed integrity checks: integrity={integrity_before} fk={fk_before}"
            )
