...
class CuAdapter:
    ...
    def _resolve_sidos(self, request: CollectionRequest) -> list[str]:
        ...
        `regions`에 이름을 주면 코드로 옮긴다. 아무것도 없으면 **화면이 주는
        지역 전부**를 하나씩 돈다.

        ── 예전에는 `AA`(전체) 한 번이었다 (2026-08-07 변경) ──────────────

        요청은 한 번으로 끝나서 싸지만, 응답 행에 지역이 없다. 이 화면은
        지역을 **조회 조건으로만** 준다. 전체로 받으면 어느 행이 어느 지역
        조합인지 영영 알 수 없다.

        그 결과 발행본에서 신협 30,994행의 지역이 전부 비었고, 화면에서
        시도를 하나라도 고르면 신협이 통째로 사라졌다. 부산 사람이 부산을
        골라도 부산 신협이 안 나왔다.

        지역으로 나눠도 총 행 수는 같다 — 같은 목록이 17조각으로 갈릴 뿐이다.
        늘어나는 것은 조각마다의 마지막 페이지 요청뿐이라, 실측 16분 46초가
        20분 안쪽으로 는다.

        한 조합이 여러 지역에 걸치면 **먼저 만난 지역**이 남는다. 기관은
        조합코드(`cuIngno`)로 하나이므로 행이 늘지는 않는다.
        """
