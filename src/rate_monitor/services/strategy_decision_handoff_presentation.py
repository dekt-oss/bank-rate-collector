...
이동할 수 있는 compact handoff는 계속 노출한다. handoff DOM은 앞선 Strategy UX
presentation이 브라우저 런타임에 생성하므로, build-time에는 UX marker를 선행
계약으로 확인한다. 계산·데이터 계약은 변경하지 않는다.
"""
...
UX_STYLE_MARKER = 'id="strategy-ux-refinement-style"'
...
def inject_strategy_decision_handoff(html: str) -> str:
    ...
    if UX_STYLE_MARKER not in html:
        raise DashboardBuildError("Strategy Search handoff UX 선행 계약을 찾지 못했다")
