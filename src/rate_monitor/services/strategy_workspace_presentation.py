...
.workspace-detail-disclosure{margin:0;border:1px solid rgba(91,47,100,.11);border-radius:15px;background:rgba(255,255,255,.025);overflow:clip}.workspace-detail-disclosure>summary{display:flex;align-items:center;justify-content:space-between;gap:16px;cursor:pointer;list-style:none;padding:15px 17px;color:#dbe7e1;font-size:11px;font-weight:820}.workspace-detail-disclosure>summary::-webkit-details-marker{display:none}.workspace-detail-disclosure>summary span{color:#718278;font-size:9px;font-weight:620}.workspace-detail-disclosure>summary:after{content:"펼치기";margin-left:auto;color:#8b7990;font-size:9px;font-weight:720}.workspace-detail-disclosure[open]>summary:after{content:"접기"}.workspace-detail-disclosure-body{display:grid;gap:12px;padding:0 10px 12px}.workspace-detail-disclosure:not([open]){box-shadow:0 10px 28px rgba(20,10,22,.07)}
...
@media(max-width:760px){
  ...
  .workspace-detail-disclosure>summary{padding:13px 14px}.workspace-detail-disclosure>summary span{display:none}.workspace-detail-disclosure-body{padding:0 7px 9px}
...
(()=>{
  ...
  function clarifyMarketEventContract(marketFlow){
    ...
    if(summary){const chip=summary.querySelector(".chip"),text=[...summary.childNodes].find(node=>node.nodeType===Node.TEXT_NODE);if(text)text.textContent="최근 30일 금리 변경 이벤트 ";if(chip)summary.appendChild(chip)}
  ...
  function wrapDetailDisclosure(nodes){
    ...
    const disclosure=document.createElement("details");disclosure.id="workspace-detail-disclosure";disclosure.className="workspace-detail-disclosure";
    disclosure.innerHTML='<summary><b>상세 판단요소</b><span>경쟁사 · Peer · 지역 · 우대조건 · 특판 · 외부 자금환경 · 데이터 근거</span></summary><div class="workspace-detail-disclosure-body"></div>';
    ordered[0].parentNode.insertBefore(disclosure,ordered[0]);const body=disclosure.querySelector(".workspace-detail-disclosure-body");ordered.forEach(node=>body.appendChild(node));return disclosure;
  }
  function buildNavigation(targets){
    ...
    const reveal=id=>{const target=$(id);if(!target)return null;const disclosure=target.closest("details.workspace-detail-disclosure");if(disclosure)disclosure.open=true;return target};
    links.forEach(link=>link.addEventListener("click",()=>{reveal(link.dataset.workspaceTarget);setActive(link.dataset.workspaceTarget)}));
    ...
    const activateHash=()=>{const hash=location.hash.slice(1),link=links.find(item=>item.dataset.workspaceTarget===hash);if(!link)return;const target=reveal(hash);setActive(hash);if(target)requestAnimationFrame(()=>target.scrollIntoView({block:"start"}))};
    const hash=location.hash.slice(1);if(links.some(link=>link.dataset.workspaceTarget===hash))activateHash();else setActive(links[0].dataset.workspaceTarget);window.addEventListener("hashchange",activateHash);
  }
  function install(){
