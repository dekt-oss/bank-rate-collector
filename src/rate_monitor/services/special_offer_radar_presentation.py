...
const evidenceLink=x=>{const href=safeHref(x.availability_source_locator||x.evidence_ref||x.evidence_source_locator);const when=x.availability_observed_at||x.evidence_observed_at||"";const assertion=String(x.availability_assertion||"").trim();return `<span class="special-radar-evidence">${assertion?`<b>${esc(assertion)}</b>`:""}${href?`<a href="${esc(href)}" target="_blank" rel="noopener noreferrer">근거 보기</a>`:"기록됨"}${when?`<small>${esc(String(when).slice(0,16).replace("T"," "))}</small>`:""}</span>`};
function offerTable(items,emptyText){if(!items.length)return `<div class="special-radar-empty"><strong>0</strong><div><b>${esc(emptyText)}</b><p>판매상태를 공식 근거로 확인한 특판만 이 탭에 표시합니다.</p></div></div>`;return `<div class="special-radar-table-wrap"><table class="special-radar-table"><thead><tr><th>금융사</th><th>상품</th><th>특판금리</th><th>판매기간</th><th>한도</th><th>가입대상</th><th>조기종료 조건</th><th>판매상태·공식근거</th></tr></thead><tbody>${items.map(x=>`<tr><td>${esc(x.institution_name)}</td><td>${esc(x.product_name)}</td><td class="rate">${specialRate(x)}</td><td>${period(x)}</td><td>${quota(x)}</td><td>${eligibility(x)}</td><td>${early(x)}</td><td>${evidenceLink(x)}</td></tr>`).join("")}</tbody></table></div>`}
...
function emptyState(){return `<div class="special-radar-empty"><strong>대기</strong><div><b>현재 확정된 특판은 0건입니다.</b><p>FSB 일반 공시는 금리정보는 주지만 특판 여부를 명시하지 않아 현재 ${Number(counts.unknown||0).toLocaleString("ko-KR")}건을 미판정으로 보존합니다. 기능 자체는 유지하며, 공식 상품페이지 등 상품 단위 명시 근거가 확보되면 Radar에 축적합니다.</p></div></div>`}
