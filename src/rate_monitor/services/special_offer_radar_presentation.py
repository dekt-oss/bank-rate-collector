...
function offersTable(){if(!offers.length)return `<div class="special-radar-empty"><strong>OFF</strong><div><b>공식 근거로 확정된 특판이 아직 없습니다.</b><p>FSB 일반 공시 화면이 특판 여부를 말하지 않은 상품은 <b>unknown</b>으로 유지합니다. “특판 표기가 없음 = 일반상품”으로 추정하지 않으며, 확정 근거가 생기기 전에는 시장 특판 벤치마크와 경쟁순위에 넣지 않습니다.</p></div></div>`;return `<div class="special-radar-table-wrap"><table class="special-radar-table"><thead><tr><th>금융사</th><th>상품</th><th>대표 최고금리</th><th>기간</th><th>채널</th><th>공식 근거</th></tr></thead><tbody>${offers.map(x=>`<tr><td>${esc(x.institution_name)}</td><td>${esc(x.product_name)}</td><td class="rate">${rate(x.representative_rate)}</td><td>${x.term_months?`${Number(x.term_months)}개월`:"—"}</td><td>${esc(x.join_channel||"—")}</td><td>${x.evidence_ref?`<a href="${esc(x.evidence_ref)}" target="_blank" rel="noopener noreferrer">근거 보기</a>`:"—"}</td></tr>`).join("")}</tbody></table></div>`}
