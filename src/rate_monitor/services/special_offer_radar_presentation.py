...
function offersTable(){if(!offers.length)return `<div class="special-radar-empty"><strong>대기</strong><div><b>현재 확정된 특판은 0건입니다.</b><p>FSB 일반 공시는 금리정보는 주지만 특판 여부를 명시하지 않아 현재 ${Number(counts.unknown||0).toLocaleString("ko-KR")}건을 미판정으로 보존합니다. 기능 자체는 유지하며, 공식 상품페이지 등 상품 단위 명시 근거가 확보되면 확정 특판만 Radar에 자동 표시합니다.</p></div></div>`;return `<div class="special-radar-table-wrap"><table class="special-radar-table"><thead><tr><th>금융사</th><th>상품</th><th>대표 최고금리</th><th>기간</th><th>채널</th><th>공식 근거</th></tr></thead><tbody>${offers.map(x=>`<tr><td>${esc(x.institution_name)}</td><td>${esc(x.product_name)}</td><td class="rate">${rate(x.representative_rate)}</td><td>${x.term_months?`${Number(x.term_months)}개월`:"—"}</td><td>${esc(x.join_channel||"—")}</td><td>${evidenceLink(x.evidence_ref)}</td></tr>`).join("")}</tbody></table></div>`}
function install(){if($("special-offer-radar"))return;const anchor=$("market-flow");if(!anchor)return;const section=document.createElement("section");section.id="special-offer-radar";section.className="card special-radar";section.dataset.radarState=offers.length?"confirmed":"pending";const badge=offers.length?`확정 특판 ${offers.length.toLocaleString("ko-KR")}건`:"공식근거 0건 · 수집 중";const title=offers.length?"시장 특판 Radar":"특판 Radar · 근거 수집 중";const copy=offers.length?"확정된 특판만 일반 경쟁순위와 분리해 시장 압력·벤치마킹 자료로 보여줍니다.":"현재 FSB 일반 공시만으로는 특판 여부를 확정할 수 없습니다. 추정하지 않고 공식 상품단위 근거를 축적 중입니다.";section.innerHTML=`<div class="special-radar-head"><div><h2>${title}</h2><p>${copy}</p></div><span class="special-radar-badge">${badge}</span></div><div class="special-radar-metrics">${metric("확정 특판",counts.confirmed_special,"Radar에 표시 가능한 근거")}${metric("확정 일반",counts.confirmed_normal,"명시적 일반상품 근거")}${metric("미판정",counts.unknown,"특판 여부를 말하지 않은 공시")}${metric("근거 충돌",counts.conflict,"자동 선택하지 않고 차단")}</div>${offersTable()}<div class="special-radar-foot"><span>기준일 <b>${esc(payload.as_of||"—")}</b> · 원천 <b>${esc(payload.source_id||"—")}</b></span><span>현재 정책: <b>확정근거 전까지 경쟁순위 미반영</b> · 일반 금리/Relative Pricing 모집단 변경 없음</span></div>`;anchor.parentNode.insertBefore(section,anchor)}
