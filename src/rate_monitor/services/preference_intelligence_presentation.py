...
D1 ``preference_intelligence`` 결과만 표시한다. 우대조건 비중의 분모는 실제
...
(()=>{
  ...
  panel.innerHTML=`<div class="pref-intel-head"><div><h2>상품 · 우대조건 전략</h2><p>우대조건 보유 상품 안에서 조건 구성을 비교합니다.</p></div><span class="pref-intel-badge">D1 Structure Evidence</span></div><div class="pref-intel-controls"><div class="pref-intel-control"><span>업권</span>${Object.entries(sectors).map(([k,v])=>`<button type="button" data-pi-sector="${k}">${v}</button>`).join("")}</div><div class="pref-intel-control"><span>기간</span>${[6,12,24,36].map(v=>`<button type="button" data-pi-term="${v}">${v}개월</button>`).join("")}</div></div><div class="pref-intel-caveat"><b>분모는 우대조건 보유 상품입니다.</b> 하나의 상품이 여러 조건에 포함될 수 있어 조건 비중 합계는 100%를 넘을 수 있습니다. 구조 비교이며 수신효과 추정이 아닙니다.</div><div id="preference-intelligence-body"></div>`;
