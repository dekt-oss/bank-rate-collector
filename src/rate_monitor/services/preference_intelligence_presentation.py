...
D1 ``preference_intelligence`` 결과만 표시한다. 상위금리 상품에서 더 자주
관찰되는 조건을 수신증가의 원인으로 해석하지 않으며, 내부 실적 보정 전에는
인과효과를 계산하거나 문구로 암시하지 않는다.
"""
...
(()=>{
  ...
  panel.innerHTML=`<div class="pref-intel-head"><div><h2>상품 · 우대조건 전략</h2><p>시장 전체와 상위금리 상품의 우대조건 구성을 비교합니다.</p></div><span class="pref-intel-badge">D1 Structure Evidence</span></div><div class="pref-intel-controls"><div class="pref-intel-control"><span>업권</span>${Object.entries(sectors).map(([k,v])=>`<button type="button" data-pi-sector="${k}">${v}</button>`).join("")}</div><div class="pref-intel-control"><span>기간</span>${[6,12,24,36].map(v=>`<button type="button" data-pi-term="${v}">${v}개월</button>`).join("")}</div></div><div class="pref-intel-caveat"><b>구조 비교이며 수신효과 추정이 아닙니다.</b> 상위금리 상품에서 더 자주 관찰된 조건이 그 상품의 수신을 늘렸다는 뜻은 아닙니다. 실제 효과는 내부 실적 보정 후 D2/E에서만 판단합니다.</div><div id="preference-intelligence-body"></div>`;
  ...
  function render(){
    ...
    const warning=coverage.coverage_status==="low"?`<div class="pref-intel-warning"><b>우대조건 제공률이 낮습니다.</b> 현재 알려진 조건 ${percent(coverage.known_preference_share)} · 미제공 ${Number(coverage.missing_count||0).toLocaleString("ko-KR")}건. 미제공을 '조건 없음'으로 해석하지 않습니다.</div>`:"";
    const rows=(item.categories||[]).slice(0,8).map(c=>`<tr><td class="${c.is_other?"other":""}">${c.label}</td><td class="mono">${percent(c.market_share)}</td><td class="mono">${percent(c.top_tier_share)}</td><td class="mono ${Number(c.top_tier_lift_pp)>0?"positive":Number(c.top_tier_lift_pp)<0?"negative":""}">${lift(c.top_tier_lift_pp)}</td></tr>`).join("")||'<tr><td colspan="4">분류 가능한 우대조건이 없습니다.</td></tr>';
    ...
    body.innerHTML=`${warning}<div class="pref-intel-grid"><div class="pref-intel-main"><div class="pref-intel-summary"><div><span>우대정보 제공률</span><b>${percent(coverage.known_preference_share)}</b></div><div><span>상위금리 기준</span><b>${Number.isFinite(Number(top.cutoff_rate))?Number(top.cutoff_rate).toFixed(2)+"%":"—"}</b></div><div><span>상위군 우대정보 제공</span><b>${percent(topCoverage.known_preference_share)}</b></div></div><table class="pref-intel-table"><thead><tr><th>조건</th><th>시장 전체</th><th>상위금리상품</th><th>차이</th></tr></thead><tbody>${rows}</tbody></table></div>${ownHtml}</div>`;
  }
