...
(()=>{
  ...
  function compactPlanning(planning){
    ...
    if(!headline){
      ...
      const strip=sim.querySelector(":scope>.planning-strip")||sim.querySelector(".planning-strip");
      (form||strip)?.insertAdjacentElement("beforebegin",headline);
