...
def _adapt_prediction_panel(text: str) -> str:
    ...
    new_detail_warning = (
        ...
        '모형을 참고했습니다. <a href="https://www.federalreserve.gov/pubs/feds/2013/201380/index.html" '
        'target="_blank" rel="noopener noreferrer">Fed FEDS 2013 · Sticky Deposit Rates</a></span>'
        ...
        '<a href="https://libertystreeteconomics.newyorkfed.org/2025/07/the-rise-in-deposit-flightiness-and-its-implications-for-financial-stability/" '
        ...
        '<a href="https://www.bok.or.kr/portal/bbs/P0002353/view.do?menuNo=200433&amp;nttId=10081072" '
        'target="_blank" rel="noopener noreferrer">한국은행 BOK 이슈노트 2023-33</a></span>'
    ...
    helper = (
        'function togglePredictionPanel(){const panel=$("prediction-panel"),button=$("prediction-toggle"),'
        'opening=panel.hidden;panel.hidden=!opening;button.setAttribute("aria-expanded",opening?"true":"false");'
...
def _adapt_national_map_readability(text: str) -> str:
    ...
    css = (
        ...
        '.engine-toggle:hover,.engine-toggle:focus-visible{border-color:rgba(128,200,166,.52);'
        'background:rgba(73,125,97,.24);outline:none}'
        ...
        '.model-evidence a{color:#9ccdb8;text-decoration:none;border-bottom:1px solid rgba(156,205,184,.25)}'
    ...
    old_nodes = (
        '  $("nodes").innerHTML=a.map(x=>{const[cx,cy]=coords[x.region],right=cx>400,'
        'lx=right?cx-22:cx+22,anchor=right?"end":"start",klass=["node",'
        'x.region===top?"top":"",x.region==="부산"?"busan clickable":""].filter(Boolean).join(" ");'
        'return`<g class="${klass}" data-region="${esc(x.region)}" role="${x.region==="부산"?"button":"img"}" '
        'tabindex="${x.region==="부산"?"0":"-1"}" aria-label="${esc(x.region)} 지역 평균 ${x.rate.toFixed(2)}%'
        '${x.region==="부산"?", 부산 지도 확대":""}"><line class="node-line" x1="${cx}" y1="${cy}" '
        'x2="${right?cx-34:cx+34}" y2="${cy-11}"/><circle class="node-ring" cx="${cx}" cy="${cy}" '
        'r="${x.region===top?14:11}"/><circle class="node-core" cx="${cx}" cy="${cy}" r="4.3"/>'
        '<text class="node-label" x="${lx}" y="${cy-16}" text-anchor="${anchor}">${esc(x.region)}</text>'
        '<text class="node-rate" x="${lx}" y="${cy-3}" text-anchor="${anchor}">${x.rate.toFixed(2)}%</text>'
        '</g>`}).join("");'
    )
    new_nodes = (
        '  $("nodes").innerHTML=a.map(x=>{const[cx,cy]=coords[x.region],preset=koreaLabelOffsets[x.region]'
        '||[22,-16,"start"],dx=preset[0],dy=preset[1],anchor=preset[2],lx=cx+dx,labelY=cy+dy,'
        'rateY=labelY+14,lineX=anchor==="end"?lx+7:anchor==="start"?lx-7:lx,lineY=labelY+5,'
        'klass=["node",x.region===top?"top":"",x.region==="부산"?"busan clickable":""].filter(Boolean).join(" ");'
        'return`<g class="${klass}" data-region="${esc(x.region)}" role="${x.region==="부산"?"button":"img"}" '
        'tabindex="${x.region==="부산"?"0":"-1"}" aria-label="${esc(x.region)} 지역 평균 ${x.rate.toFixed(2)}%'
        '${x.region==="부산"?", 부산 지도 확대":""}"><line class="node-line" x1="${cx}" y1="${cy}" '
        'x2="${lineX}" y2="${lineY}"/><circle class="node-ring" cx="${cx}" cy="${cy}" '
        'r="${x.region===top?14:11}"/><circle class="node-core" cx="${cx}" cy="${cy}" r="4.3"/>'
        '<text class="node-label" x="${lx}" y="${labelY}" text-anchor="${anchor}">${esc(x.region)}</text>'
        '<text class="node-rate" x="${lx}" y="${rateY}" text-anchor="${anchor}">${x.rate.toFixed(2)}%</text>'
        '</g>`}).join("");'
    )
