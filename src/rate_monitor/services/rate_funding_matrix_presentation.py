...
function currentRateCount(d){return Number(d.current_rate_institutions??d.current_rate_institutions_not_carried_back??0)}
function meta(d){return [kpi("Funding 6M 비교",`${Number(d.funding_growth_6m_institutions||0).toLocaleString("ko-KR")}개`,`${esc(d.analysis_month)} 기준`),kpi("동일시점 금리",`${Number(d.historical_rate_institutions||0).toLocaleString("ko-KR")}개`,`12M 정기예금 공시 최고금리`),kpi("정확 결합",`${Number(d.paired_institutions||0).toLocaleString("ko-KR")}개`,d.pair_coverage_ratio?`결합률 ${pct(d.pair_coverage_ratio)}`:"결합률 산정 불가"),kpi("현재 금리",`${currentRateCount(d).toLocaleString("ko-KR")}개`,`과거 기준월에 소급하지 않음`)].join("")}
function statusLabel(d){if(d.available)return "시점정합 Matrix";if(d.status==="rate_data_unavailable")return "금리 데이터 미수집";if(d.status==="insufficient_exact_pairs")return "exact pair 부족";return "과거금리 이력 부족"}
function blocked(d){if(d.status==="rate_data_unavailable")return `<div class="rate-funding-matrix-blocked"><b>이 업권의 12개월 공시금리 데이터가 없어 Matrix를 열지 않습니다.</b><p>${esc(d.analysis_month)} 수신잔액 비교 데이터는 있지만 동일 기관에 연결된 현재 12개월 정기예금 공시 최고금리 데이터가 없습니다. 이름 유사 매칭이나 다른 업권 금리를 대신 붙이지 않습니다. canonical 금리 수집이 연결되기 전에는 Matrix를 생성하지 않습니다.</p></div>`;if(d.status==="insufficient_exact_pairs")return `<div class="rate-funding-matrix-blocked"><b>동일시점 exact pair 표본이 부족해 Matrix를 열지 않습니다.</b><p>${esc(d.analysis_month)} 기준 금리와 6M 수신증감이 모두 있는 동일 기관 exact pair가 충분하지 않습니다. missing을 0으로 채우거나 다른 월을 보간하지 않습니다.</p></div>`;return `<div class="rate-funding-matrix-blocked"><b>시점정합 금리 이력이 부족해 Matrix를 열지 않습니다.</b><p>${esc(d.analysis_month)} 수신잔액과 같은 시점에 유효했던 12개월 정기예금 공시 최고금리를 찾지 못했습니다. 현재 공시금리가 존재하더라도 과거 수신잔액에 소급해 붙이지 않습니다. 동일시점 exact pair가 확보되면 이 영역은 자동으로 사분면 Matrix로 전환됩니다.</p></div>`}
