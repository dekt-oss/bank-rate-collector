...
special-sale status from explicit product-level evidence.  The radar therefore
uses ``resolve_special_offer_state`` and never treats ``unknown`` as a promotion.
It is a read-only Strategy payload; activating a public promotion benchmark remains
...
def _product_context(conn: sqlite3.Connection, product_id: str) -> dict[str, Any] | None:
    row = conn.execute(
...
def _current_fsb_rate(conn: sqlite3.Connection, product_id: str) -> dict[str, Any]:
    placeholders = ",".join("?" for _ in _CONFIRMED_RUN_STATUSES)
    rows = conn.execute(
        "SELECT ro.max_rate, ro.base_rate, ro.source_effective_at, ro.last_seen_at, "
        "pv.term_months, pv.join_channel "
        ...
        "WHERE pv.product_id=? AND cr.source_id=? "
        f"AND cr.status IN ({placeholders}) AND ro.validation_status='valid'",
        (product_id, SOURCE_ID, *_CONFIRMED_RUN_STATUSES),
    ).fetchall()
    candidates: list[tuple[float, str, Any, Any, Any]] = []
    for max_rate, base_rate, source_effective_at, last_seen_at, term_months, join_channel in rows:
...
def build_special_offer_radar(
    ...
    *,
    ...
    try:
        ...
        product_context = {
            product_id: _product_context(conn, product_id) for product_id in product_ids
        }
        rates = {
            product_id: _current_fsb_rate(conn, product_id) for product_id in product_ids
        }
    finally:
        conn.close()
