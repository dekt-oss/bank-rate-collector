...
(()=>{
  ...
  function install(){
    ...
    caveat.innerHTML="<b>내부 실적 미보정 스트레스 시나리오</b> — 현재 계수는 고려저축은행 실적 추정치가 아니며 실제 예측치가 아닙니다. 내부 실적 확보 후 별도 보정 단계를 거칩니다.";
    panel.insertBefore(caveat,panel.firstChild);

    const inputs=panel.querySelector(".predict-inputs");
    const disclosure=document.createElement("details");
    disclosure.className="rate-response-disclosure";
    disclosure.innerHTML='<summary><b>금리별 수신반응 상세</b><span>+5bp · +10bp · +15bp · 제안금리 비교</span></summary><div class="rate-response-disclosure-body"></div>';
    ...
    disclosure.querySelector(".rate-response-disclosure-body").appendChild(wrap);
    if(inputs)inputs.insertAdjacentElement("afterend",disclosure);else panel.appendChild(disclosure);
