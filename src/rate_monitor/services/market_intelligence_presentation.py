...
.market-intel-counts{display:flex;gap:6px;flex-wrap:wrap;margin-top:9px}.market-intel-count{display:inline-flex;align-items:center;gap:5px;padding:6px 8px;border:1px solid rgba(213,225,219,.09);border-radius:8px;background:rgba(255,255,255,.035);font-size:9.5px;font-weight:800;line-height:1}.market-intel-count b{font:850 12px/1 var(--mono)}.market-intel-count.up{color:#61b890;border-color:rgba(97,184,144,.23);background:rgba(97,184,144,.08)}.market-intel-count.down{color:#d97878;border-color:rgba(217,120,120,.23);background:rgba(217,120,120,.08)}.market-intel-count.flat{color:#9c91a0}.market-intel-count i{font-style:normal;font-size:11px}
  ...
  function render(){
    ...
    body.innerHTML=`
      <div class="market-intel-status">
        <div class="market-intel-direction ${dir}"><span>${sectors[state.sector]} · ${state.term}개월 · ${state.window}D</span><strong>${directions[dir]||dir}</strong><small>실제 관측 ${Number(item.observed_days).toFixed(1)}일 · 비교상품 ${total.toLocaleString("ko-KR")}개</small><div class="market-intel-counts" aria-label="금리 변경 건수"><span class="market-intel-count up"><i aria-hidden="true">↑</i> 인상 <b>${up.toLocaleString("ko-KR")}</b>건</span><span class="market-intel-count down"><i aria-hidden="true">↓</i> 인하 <b>${down.toLocaleString("ko-KR")}</b>건</span><span class="market-intel-count flat"><i aria-hidden="true">→</i> 유지 <b>${flat.toLocaleString("ko-KR")}</b>건</span></div></div>
        <div class="market-intel-metrics">
          ...
          <div class="market-intel-metric"><span>금리변경 확산도</span><b>${Number(item.breadth_score)>0?"+":""}${Number(item.breadth_score).toFixed(2)}</b><small>인상 ${up} · 인하 ${down} · 유지 ${flat}</small></div>
