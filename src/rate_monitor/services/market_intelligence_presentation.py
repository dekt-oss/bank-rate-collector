...
.market-intel-status{display:grid;grid-template-columns:minmax(270px,1fr) minmax(0,2fr);gap:10px;margin-bottom:10px}.market-intel-direction{padding:13px;border:1px solid rgba(213,225,219,.08);border-radius:12px;background:rgba(7,19,16,.38)}.market-intel-direction span{display:block;color:#71847a;font-size:9px}.market-intel-direction strong{display:block;margin-top:3px;color:#dce9e2;font-size:22px;letter-spacing:-.04em}.market-intel-direction small{display:block;margin-top:4px;color:#708178;font-size:9px;line-height:1.45}.market-intel-event-counts{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:6px;margin-top:10px}.market-intel-event-count{min-width:0;padding:8px 7px;border:1px solid rgba(213,225,219,.08);border-radius:9px;background:rgba(255,255,255,.025)}.market-intel-event-count span{display:flex;align-items:center;gap:5px;color:#81938a;font-size:9px;font-weight:740}.market-intel-event-count b{display:block;margin-top:3px;color:#d8e4de;font:820 16px/1 var(--mono)}.market-intel-event-count.up span,.market-intel-event-count.up b{color:#4c9a76}.market-intel-event-count.down span,.market-intel-event-count.down b{color:#c76d6d}.market-intel-event-count.flat span,.market-intel-event-count.flat b{color:#7b8580}.market-intel-direction.rising strong{color:var(--green)}.market-intel-direction.falling strong{color:#d98989}.market-intel-direction.mixed strong{color:var(--gold)}
...
.market-intel-breadth{display:grid;grid-template-columns:auto minmax(120px,1fr) minmax(260px,auto);align-items:center;gap:9px;padding:9px 11px;border:1px solid rgba(213,225,219,.07);border-radius:11px;background:rgba(4,14,11,.22);color:#71847a;font-size:9px}.market-intel-breadth .event-breakdown{font-weight:720;white-space:nowrap}.market-intel-breadth .event-breakdown .up{color:#4c9a76}.market-intel-breadth .event-breakdown .down{color:#c76d6d}.market-intel-breadth .event-breakdown .flat{color:#7b8580}.market-intel-breadth strong{color:#b7c9c0}.market-intel-bar{display:flex;height:8px;border-radius:99px;overflow:hidden;background:rgba(255,255,255,.05)}.market-intel-bar i{display:block;height:100%}.market-intel-bar .up{background:#80c8a6}.market-intel-bar .flat{background:#687b72}.market-intel-bar .down{background:#d98989}.market-intel-period{margin-top:8px;color:#65786e;font-size:9px;line-height:1.45}.market-intel-period b{color:#91a79c}.market-intel-empty{padding:18px 13px;border:1px dashed rgba(212,179,111,.22);border-radius:11px;background:rgba(73,55,24,.08);color:#ae9d75;font-size:9.5px;line-height:1.6;text-align:center}
  ...
  function render(){
    ...
    body.innerHTML=`
      <div class="market-intel-status">
        <div class="market-intel-direction ${dir}"><span>${sectors[state.sector]} · ${state.term}개월 · ${state.window}D</span><strong>${directions[dir]||dir}</strong><small>실제 관측 ${Number(item.observed_days).toFixed(1)}일 · 비교상품 ${total.toLocaleString("ko-KR")}개</small><div class="market-intel-event-counts" aria-label="금리변경 건수"><div class="market-intel-event-count up"><span>▲ 인상</span><b>${up.toLocaleString("ko-KR")}건</b></div><div class="market-intel-event-count down"><span>▼ 인하</span><b>${down.toLocaleString("ko-KR")}건</b></div><div class="market-intel-event-count flat"><span>─ 유지</span><b>${flat.toLocaleString("ko-KR")}건</b></div></div></div>
        <div class="market-intel-metrics">
          ...
          <div class="market-intel-metric"><span>인상-인하 균형</span><b>${Number(item.breadth_score)>0?"+":""}${Number(item.breadth_score).toFixed(2)}</b><small>-1 인하 우세 · +1 인상 우세</small></div>
      ...
      <div class="market-intel-breadth"><strong>변경상품 구성</strong><div class="market-intel-bar" aria-label="인상 유지 인하 비중"><i class="up" style="width:${upW}%"></i><i class="flat" style="width:${flatW}%"></i><i class="down" style="width:${downW}%"></i></div><span class="event-breakdown"><span class="up">▲ 인상 ${up.toLocaleString("ko-KR")}건 (${pct(item.up_share)})</span> · <span class="down">▼ 인하 ${down.toLocaleString("ko-KR")}건 (${pct(item.down_share)})</span> · <span class="flat">─ 유지 ${flat.toLocaleString("ko-KR")}건</span></span></div>
