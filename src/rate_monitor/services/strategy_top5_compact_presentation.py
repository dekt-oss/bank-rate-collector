...
(()=>{
  ...
  function decorateScope(card){
    const head=card?.querySelector(".head"),title=head?.querySelector("h2"),copy=head?.querySelector("p");
    if(!head)return;
    if(title&&title.textContent!=="경쟁사 최고금리 TOP 5")title.textContent="경쟁사 최고금리 TOP 5";
    let scope=head.querySelector(".strategy-top5-scope");
    if(!scope){
      scope=document.createElement("div");scope.className="strategy-top5-scope";
      head.insertAdjacentElement("afterend",scope);
    }
    const raw=String(copy?.textContent||"").trim();
    scope.innerHTML='<b>비교조건</b><span>'+raw+'</span><small>동일 조건 내 대표 최고금리 순</small>';
  ...
  function decorate(){
    ...
    card.dataset.top5Compact="1";decorateScope(card);addHeader(card);
